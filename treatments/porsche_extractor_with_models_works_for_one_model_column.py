# porsche_extractor_FINAL_WORKING.py
# גרסה סופית עובדת - זיהוי נכון של דגמים!
...
def find_vertical_model_columns(page, measures_y, first_service_x):
    """
    🔴 חיפוש טקסט דגמים - למעלה מ-Measures וקרוב לעמודת service הראשונה
    """
    all_text = extract_all_text_with_rotation(page)

    print(f"\n  🔍 Searching for model column text...")
    ...
    for item in all_text:
        # 🔴 תנאי 1: למעלה מ-Measures (Y קטן יותר!)
        if item["y"] >= measures_y:
            continue

        # 🔴 תנאי 2: קרוב לעמודת service הראשונה (10-50 פיקסלים משמאל)
        distance_from_first_service = first_service_x - item["x"]
        if distance_from_first_service < 10 or distance_from_first_service > 50:
            continue

        # תנאי 3: לא מילים נפוצות
    ...
    for c in model_candidates[:5]:
        print(f"       \"{c['text']}\" (X={c['x']:.1f}, Y={c['y']:.1f})")

    # מיין לפי קרבה לעמודת service
    model_candidates.sort(key=lambda i: i["distance_from_service"])

    # חשב X ממוצע
    avg_x = sum(i["x"] for i in model_candidates) / len(model_candidates)
...
def map_services_with_models_CV_V2(pdf_path, text_data, dots_data, service_type):
    ...
    print(f"\n📋 Mapping with model detection + CV...")
    ...
    print(f"\n  🔗 Cross-referencing {len(dots_data)} CV dots...")
    ...
    for item in text_data:
        ...
        if group_x in service_cols:
            ...
            if not applicable_models and model_cols:
                applicable_models = list(model_cols.values())
            elif not applicable_models:
...
def main():
    print("=" * 80)
    print("🚗 PORSCHE EXTRACTOR - FINAL WORKING VERSION!")
    print("=" * 80)
